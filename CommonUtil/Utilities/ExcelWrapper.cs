...
namespace CommonUtil.Utilities
{
    public class ExcelWrapper
    {
        ...
        private void openWorkbook()
        {
            ...
            if(_pathAndFile.ToUpper().EndsWith("XLS"))
                _workBook = new HSSFWorkbook(_underlyingStream, true);
            else
                _workBook = new XSSFWorkbook(_underlyingStream);
        ...
        public void CreateFile(string pathAndFile)
        {
            ...
            if (_pathAndFile.ToUpper().EndsWith("XLS"))
                _workBook = new HSSFWorkbook(); // HSSFWorkbook();
            else
        ...
        /// <summary>
        /// Gets the numeric cell value of the cell
        /// Adeoye Remi 30/12/2013 5:56pm
        /// </summary>
        /// <param name="row">0-based row number</param>
        /// <param name="column">0-based column number</param>
        /// <returns>String representation of cell value</returns>
        public Decimal GetCellNumericValue(int row, int column)
        {
            if (_workBook == null)
        ...
        /// <summary>
        /// Gets the Column index where search value is first found
        /// Adeoye Remi 1/1/2014 12.39am
        /// </summary>
        /// <param name="row">0-based row number</param>
        /// <param name="searchString">String to search for</param>
        /// <returns>String representation of cell value</returns>
        public ExcelCell FindInRow(string searchString, int row)
        {
            if (_workBook == null)
                throw new Exception("Error - workbook is not initialized");
            if (_workSheet == null)
            ...
            {
                int limit = 256;
                ExcelCell found_cell = null;

                for (int i = 0; i < limit; i++)
                {
                    ExcelCell excelCell = new ExcelCell(_workBook, _workSheet , row, i);
                    if (excelCell.IsValidCell)
                    {
                        string match = this.GetCellValue(row, i);
                        if (match == searchString)
                        {
                            found_cell = excelCell;
                            break;
                        }
                    }
                }
        ...
        /// <summary>
        /// Public access to Locked Status of cell
        /// Adeoye Remi 31/12/2013 9.42pm
        /// </summary>
        public bool IsLocked(int row, int column)
        {
            ...
            try
            {
                return _workSheet.GetRow(row).GetCell(column).CellStyle.IsLocked;
            }
            catch
            {
                return false;
        ...
        public bool CellIsHidden(string cellName)
        {
            ExcelCell cell = new ExcelCell(_workBook, _workSheet, cellName);
    ...
    public class CellStyleFactory
    {
        ...
        /// <summary>
        /// Converts wrapper border style to NPOI border style
        /// </summary>
        /// <param name="borderType">Wrapper border type</param>
        /// <returns>NPOI border type</returns>
        public static BorderStyle GetBorderStyle(BorderTypes borderType)
        {

            if (borderType == BorderTypes.Dashed)
                return BorderStyle.Dashed;
            else if (borderType == BorderTypes.Dotted)
                return BorderStyle.Dotted;
            else if (borderType == BorderTypes.Double)
                return BorderStyle.Double;
            else if (borderType == BorderTypes.Hair)
                return BorderStyle.Hair;
            else if (borderType == BorderTypes.Medium)
                return BorderStyle.Medium;
            else if (borderType == BorderTypes.Thick)
                return BorderStyle.Thick;
            else if (borderType == BorderTypes.Thin)
        ...
        public static short GetColor(ExcelColors color)
        {
            if (color == ExcelColors.Black)
                ...
                return NPOI.HSSF.Util.HSSFColor.LightBlue.Index;
            else if (color == ExcelColors.LightGreen)
                return NPOI.HSSF.Util.HSSFColor.LightGreen.Index;
            else if (color == ExcelColors.LightYellow)
                return NPOI.HSSF.Util.HSSFColor.LightYellow.Index;
    ...
    public class ExcelCell
    {
        ...
        public ExcelCell(IWorkbook workbook, ISheet worksheet, string excelName)
        {
            _workBook = workbook;
        ...
        /// <param name="row">0-based row number</param>
        /// <param name="column">0-based column number</param>
        public ExcelCell(IWorkbook workbook, ISheet worksheet, int row, int column)
        {
            _workBook = workbook;
            _style = new ExcelStyle();
            _workSheet = worksheet;
        ...
        public Decimal Value2
        {
            get
            {
                try
                {
                    ...
                    Decimal.TryParse(_cell.NumericCellValue.ToString(), out decimalValue);
                    return decimalValue;
                }
                catch
                {
        ...
        public void WriteCellValue(string value)
        {
            ...
            if (!_style.Equals(ExcelStyle.GetGenericStyle()))
                _cell.CellStyle = _style.ToCellStyle(_workBook);
    ...
    public class ExcelStyle
    {
        ...
        public ExcelStyle()
        {
            _usedStyles = new List<ICellStyle>();
            this.IsBold = false;
        ...
        /// <summary>
        /// Gets a NPOI cell style
        /// </summary>
        /// <param name="wrapper">The main excel wrapper</param>
        /// <returns>NPOI Cell Style</returns>
        public ICellStyle ToCellStyle(IWorkbook workbook)
        {
            ICellStyle returnStyle = workbook.CreateCellStyle();
            returnStyle.Alignment = CellStyleFactory.GetAlignment(this.Alignment);
            returnStyle.WrapText = this.WrapText;

            if (this.BorderBottom)
                returnStyle.BorderBottom = CellStyleFactory.GetBorderStyle(this.BorderType);
            if (this.BorderLeft)
                returnStyle.BorderLeft = CellStyleFactory.GetBorderStyle(this.BorderType);
            if (this.BorderRight)
                returnStyle.BorderRight = CellStyleFactory.GetBorderStyle(this.BorderType);
            if (this.BorderTop)
                returnStyle.BorderTop = CellStyleFactory.GetBorderStyle(this.BorderType);

            IFont font = workbook.CreateFont();
            font.FontName = this.FontFace.ToString();
            ...
            if (this.IsUnderlined)
                //font.Underline = NPOI.SS.UserModel.FontUnderline.SINGLE.ByteValue;
            font.FontHeightInPoints = (short)this.FontSize;
            returnStyle.SetFont(font);
            returnStyle.FillBackgroundColor = CellStyleFactory.GetColor(this.BackColor);

            if (this.BackColor != ExcelColors.White)
                returnStyle.FillPattern = FillPattern.SolidForeground;
            returnStyle.FillForegroundColor = CellStyleFactory.GetColor(this.ForeColor);
